...
import { DifficultyLevel, Symbol } from "@tictac/domain";
...
function Menu() {
  ...
  function selectX(): void {
 
    setSymbol(Symbol.X)
  }
  function selectO(): void {
  ...
  function renderDifficultyLevel() {
    return (
      <div className={`flex items-center justify-center text-3xl ${multiplayer ? "text-gray-600" : "text-white"}`}>
            Difficulty Level:
            <button  onClick={() => toggleDifficultyLevel(difficultyLevel)} className={`rounded bg-gray-500 px-6 m-4 ${ multiplayer ? "bg-gray-800" : "bg-gray-500"}`}>
              { difficultyLevel }  
            </button>
      </div>
    )
...
<div>
  <div className="w-screen h-screen bg-gray-800 flex flex-col items-center justify-center">
    ...
    <div className="flex flex-row items-center justify-center mt-12 gap-10">
      ...
      <button onClick={selectX} className={`${ symbol === Symbol.X ? "bg-blue-500" : "bg-green-500"} rounded-lg hover:bg-blue-500 p-6 text-4xl font-extrabold text-white transition-colors duration-300`}>
        X
      </button>
      <button onClick={selectO} className={`${ symbol === Symbol.O ? "bg-blue-500" : "bg-green-500"} rounded-lg hover:bg-blue-500 p-6 text-4xl font-extrabold text-white transition-colors duration-300`}>
        O
      </button>
    </div>
    <div className="flex items-center justify-center mt-8 text-3xl text-white">
          Number of players:
          <button onClick={() => toggleMultiplayer(multiplayer)} className="rounded bg-gray-500 px-4 m-4">
            { multiplayer ? '2P': '1P'}  
          </button>
    </div>
    { renderDifficultyLevel() }
    <div className="mt-16">
      <button onClick={start} className="rounded-lg hover:bg-blue-500 bg-black p-4 px-16 text-white text-2xl font-semibold transition-colors duration-300">
        Start
      </button>
    </div>
  </div>
