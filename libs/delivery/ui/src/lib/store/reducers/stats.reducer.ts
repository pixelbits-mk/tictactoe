...
import { MAKE_MOVE_SUCCESS, QUIT_GAME_SUCCESS, START_GAME, START_GAME_SUCCESS } from "../actions";
...
export const statsReducer = (state = initialState, action: Action) => {
    ...
    switch (action.type) {
        case MAKE_MOVE_SUCCESS:
            const gameState = (action as any).state as GameState
            const player1Win = gameState.winner && gameState.winner.symbol == gameState.players[0].symbol ? 1 : 0
            const player2Win = gameState.winner && gameState.winner.symbol === gameState.players[1].symbol ? 1 : 0
