...
describe('game-board', () => {
  ...
  it('should set a given symbol at the specified indices', () => {
    ...
    for (let i = 0; i < 3; ++i) {
      for (let j = 0; j < 3; ++j) {
        state = gameBoard.set(state, [i, j], symbols[i * 3 + j])
      }
    }
    expect(state.positions).toEqual([
      ['1', '2', '3'],
  ...
  it('should not allow an empty symbol to be set for any position', () => {
    const state = gameBoard.initialize({ dimension: 3 })
    expect(() => gameBoard.set(state, [0, 0], '')).toThrow(
      'Cannot set an empty symbol on the board'
    )
    expect(() => gameBoard.set(state, [0, 0], '  ')).toThrowError(
      'Cannot set an empty symbol on the board'
    )
  })

  it('should get a symbol from the board', () => {
    let state = gameBoard.initialize({ dimension: 3 })
    state = gameBoard.set(state, [1, 1], 'X')

    expect(gameBoard.get(state, [1, 1])).toBe('X')
  })
})
